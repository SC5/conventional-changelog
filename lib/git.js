...
function parseRawCommit(raw, options) {
  ...
  msg.pullrequest = msg.pullrequest.match(/^Merge pull request #(\d+)/)
  if (msg.pullrequest && msg.pullrequest[1]) {
    msg.pullrequest = msg.pullrequest[1];
  } else {
    // not a PR
    return;
  }
